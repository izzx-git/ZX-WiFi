...
v.1.6 - Вместо U2, U6, Q1, J1 поставлена ATF16V8B (GAL16V8). Добавлен сигнал MREQ, для лучшей работы на Эволюции.
	Порт выбирается перепрошивкой GAL.
	Сигнал сброса на 16C550 теперь не идёт. Убран резистор подтяжки R14.
...
Внимание.
На Evolution двойная конф. в режиме TS conf работа не гарантируется.
Moon Rabbit и AY232K работают, но могут быть сбои. Также проблемы с кнопкой сброс F12 и с заходом в настройки sh+F12! 
Hard сброс (ctrl+alt+del) работает нормально. 
Если выключить ESP переключателем в режим 232 (перемычку с X3 обязательно убрать), то проблем нет. Но и wifi тоже будет отключен.
Можно выключатель с карты убрать и вывести из корпуса два провода, включая только часть схемы для ESP, когда загружена Base conf.
